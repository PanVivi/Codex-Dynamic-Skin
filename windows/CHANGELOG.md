...
- 产品显示名称统一为「Codex 动态壁纸」；管理器新增多选「添加壁纸」、可操作空状态，并显示 Codex、壁纸服务和当前壁纸状态。
- Windows 文档将普通用户的单文件 EXE 路径与开发者的源码脚本路径明确分开。
- 新增自包含的 Windows 图形化 EXE 管理器，提供壁纸库浏览与预览、一键应用、透出调节、启动/暂停/恢复、托盘控制和可选开机启动；发布文件内置动态壁纸引擎与 Node.js，最终用户无需手动运行脚本。
